...
from telebot import TeleBot from telebot.types import ReplyKeyboardMarkup, KeyboardButton, ReplyKeyboardRemove from config import ADMIN_ID, PLANES, PLANES_PRECIOS, SERVER_PUBLIC_IP from storage import load_users, save_users from utils import generate_keypair, get_next_available_ip, generate_conf, generate_qr_code, delete_conf from datetime import datetime, timedelta import os import json
...
def show_admin_menu(bot: TeleBot, chat_id: int): kb = ReplyKeyboardMarkup(resize_keyboard=True) kb.add(KeyboardButton("📦 Crear configuración")) kb.add(KeyboardButton("🗑 Eliminar configuración")) kb.add(KeyboardButton("📄 Ver configuraciones activas")) kb.add(KeyboardButton("📊 Ver estadísticas")) kb.add(KeyboardButton("📁 Respaldar datos (.json)")) kb.add(KeyboardButton("🔙 Salir"))
